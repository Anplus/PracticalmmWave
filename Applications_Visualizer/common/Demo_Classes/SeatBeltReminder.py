...
###################################################################################################
# Seat Belt Reminder
# TODO: MODIFY THIS TO BE A GENERAL LPD DEMO 
# TODO: TWO CHILD CLASSES FROM LPD DEMO TO BE SBR AND CPD 
###################################################################################################

class SeatBeltReminder():
    def __init__(self, CPD = False):
    ...
    def initStatusPane(self):
        ...
        if self.CPD:
            self.childLabels = []
            self.childStatusList = [0,0,0,0,0]
            for i in range(5):
                childLabel = QLabel()
                childPixmap = QPixmap('images/baby-sitting.png')
                childPixmap = childPixmap.scaledToWidth(50)
                childLabel.setPixmap(childPixmap)
                childLabel.setParent(self.statusImgLabel)
                self.childLabels.append(childLabel)
            self.childLabels[0].move(75,110) # Driver Front Seat
    ...
    def updateGraph(self, outputDict):
        ...
        if('OccPredictions' in outputDict):
            if not self.CPD: # PROCESSING FOR SBR
                # RESHAPE
                scoreOcc = [] #CLEAR
        ...
        for box in self.plot3D.boundaryBoxViz:
            ...
            elif box['name'][0:2] in ChildHighlightZones:
                #CHILD
                self.plot3D.changeBoundaryBoxBold(box,True, True)
